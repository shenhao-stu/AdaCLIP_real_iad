...
def main():
    ...
    for cls_name, items in test_data.items():
        ...
        # Create class dir in output
        class_dir = os.path.join(args.output_root, cls_name)
        ...
        for item in tqdm(items):
            ...
            # Symlink image (if not exists)
            if not os.path.exists(dst_img_path):
                if os.path.exists(src_img_path):
                    # Use absolute path for symlink to avoid issues
                    src_abs = os.path.abspath(src_img_path)
                    try:
                        os.symlink(src_abs, dst_img_path)
                    except OSError as e:
            ...
            if specie == 'OK':
                # Normal sample
                item['anomaly'] = "0"
                item['mask_path'] = "" # User: "output null in mask image part"
                new_items.append(item)
                continue
            
            # NG Sample - Predict
            if not os.path.exists(src_img_path):
                # Cannot predict without image
                new_items.append(item)
                continue
            ...
            try:
                ...
                with torch.no_grad():
                    # AdaCLIP forward
                    # aggregation=True returns (anomaly_map, anomaly_score)
                ...
                # Update item anomaly label (0 or 1) based on score
                # Threshold at 0.5 (AdaCLIP score is probability-like softmax output)
                item['anomaly'] = "1" if anomaly_score > 0.5 else "0"

                # Post-process Map
                anomaly_map = anomaly_map.cpu().numpy()
                if len(anomaly_map.shape) == 3:
                    anomaly_map = anomaly_map[0] # (H, W)
                
                # Gaussian blur (sigma=4 as in evaluation)
